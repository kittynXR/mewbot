...
{
    client.send_message(channel, "Usage: !setactivegames <redeem_name> <game1> [game2] [game3] ...").await?;
    return Ok(());
}
    ...
    if updated {
        ...
        client.send_message(channel, &format!("Active games for '{}' set to: {}", redeem_name, games.join(", "))).await?;
    } else {
