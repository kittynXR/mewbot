...
impl RedeemManager {
    pub fn new(twitch_manager: Arc<TwitchManager>, ai_client: Arc<AIClient>) -> Self {
        ...
        handlers.insert("Coin Game".to_string(), Box::new(CoinGameAction::new(coin_game_state.clone(), ai_client.clone(), api_client)) as Box<dyn RedeemHandler + Send + Sync>);
        handlers.insert("mao mao".to_string(), Box::new(AskAIAction::new(ai_client.clone())) as Box<dyn RedeemHandler + Send + Sync>);
        handlers.insert("toss pillo".to_string(), Box::new(vrc_osc_redeems.clone()) as Box<dyn RedeemHandler + Send + Sync>);
        handlers.insert("cream pie".to_string(), Box::new(vrc_osc_redeems.clone()) as Box<dyn RedeemHandler + Send + Sync>);
        handlers.insert("water balloon".to_string(), Box::new(vrc_osc_redeems.clone()) as Box<dyn RedeemHandler + Send + Sync>);
        handlers.insert("cat trap".to_string(), Box::new(vrc_osc_redeems.clone()) as Box<dyn RedeemHandler + Send + Sync>);
        handlers.insert("snowball".to_string(), Box::new(vrc_osc_redeems.clone()) as Box<dyn RedeemHandler + Send + Sync>);
        ...
        Self {
            ...
            handlers,
            coin_game_state,
            osc_configs: twitch_manager.get_osc_configs(),  // Add this line
        }
    }
